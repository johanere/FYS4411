...
\begin{center}\fbox{\parbox{\textwidth}{{\textbf{Algorithm: The "blocking" method}}
\begin{enumerate}
\item Initialize algorithm
\subitem - Estimate $var(\bar X)$
\subitem - Set $i=0$
\item Iterative "blocking" while size of $X_{i+1}\geq 2$:
\subitem - Compute $\hat \sigma_i^2, \hat y_i$
\subitem - Transform data $\vec X_i \rightarrow \vec X_{i+1}$, and set $i=i+1$
\item Calculate final values
\subitem - Compute $M_j$ from $\hat \sigma_j^2, \hat y_j$
\subitem - Find smallest $k$ such that $M_k \leq q_{d-k} (1-\alpha)$
\subitem - Calculate $\hat var (\bar X) = \hat \sigma_k^2/n_k$ 
\end{enumerate}}}\end{center}
