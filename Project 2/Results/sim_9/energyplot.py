...
for file in glob.glob("*.dat"):
    ...
    #plot
    start_plot=4#int(round(step*0.0005))
    if file_number == 8 or file_number == 0 or file_number == 111 or file_number == 114:
        pass
    else:
        print("kk")
        plt.plot(mcs[start_plot:],E[start_plot:],label="$\eta=%.1f $"%(label[file_number]))
    file_number+=1


plt.axhline(y=0.5, label="Exact",color='k', linestyle='-',)

plt.axhline(y=0.51, color='k', linestyle='-',alpha=0.1)

plt.legend(frameon=False, loc="best")
plt.grid("on")
plt.xlabel(r' iterations ')#, fontsize=10)
plt.ylabel(r'$\bar{E}$')#,fontsize=10)
plt.savefig("GS_eta.pdf")
plt.show()
