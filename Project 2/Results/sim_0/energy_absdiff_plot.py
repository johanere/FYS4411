...
for file in glob.glob("*.dat"):
    ...
    plt.plot(np.log2(mcs[start_plot:]),np.abs(E_avr[start_plot:]-E_avr[-1]),label="%s"%(label[file_number]))
    file_number+=1


plt.axhline(y=0.0, label="Exact",color='k', linestyle='-',)
plt.grid("on")
plt.xlabel(r'$log_2$ of MC cycles ')#, fontsize=10)
plt.ylabel(r'$|\bar{E}(cycle)-\bar{E}(2^{21})|$')#,fontsize=10)
plt.savefig("initial_energies_abs.pdf")
plt.show()
